...
(function () {
  ...
  function patchMermaidInitialize() {
    ...
    window.mermaid.initialize = function initializeWithDraftDefaults(config) {
      ...
      return originalInitialize.call(this, {
        ...
        // Better defaults for dense/complex application diagrams (tighter layout + scaling support)
        flowchart: {
          ...(safeConfig.flowchart || {}),
          useMaxWidth: true,
          nodeSpacing: 25,
          rankSpacing: 35,
          curve: 'basis'
  ...
  function ensureStyles() {
    ...
    style.textContent = `
      ...
      /* Support zooming out on dense/complex diagrams so more content is visible (smaller but complete view) */
      .diagram-slot svg { max-width: 100%; height: auto; width: 100%; display: block; margin: 0 auto; }
